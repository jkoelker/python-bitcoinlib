...
class CBloomFilter(object):
    ...
    def __init__(self, nElements, nFPRate, nTweak, nFlags):
        ...
        v_data = int(min(-1 / LN2SQUARED * nElements * math.log(nFPRate),
                         self.MAX_BLOOM_FILTER_SIZE * 8) / 8)
        self.vData = bytearray(v_data)
        self.nHashFuncs = int(min(len(self.vData) * 8 / nElements * LN2,
                                  self.MAX_HASH_FUNCS))
        self.nTweak = nTweak
        self.nFlags = nFlags

    def bloom_hash(self, nHashNum, vDataToHash):
        hash_seed = ((nHashNum * 0xFBA4C795) + self.nTweak) & 0xFFFFFFFF
        return MurmurHash3(hash_seed, vDataToHash) % (len(self.vData) * 8)
    ...
    def insert(self, elem):
        ...
        elem may be a core.COutPoint or bytes
        """
        if isinstance(elem, core.COutPoint):
            elem = elem.serialize()
    ...
    def IsWithinSizeConstraints(self):
        return (len(self.vData) <= self.MAX_BLOOM_FILTER_SIZE and
                self.nHashFuncs <= self.MAX_HASH_FUNCS)
    ...
    def deserialize(self, f):
        self.vData = serialize.deser_string(f)
        (self.nHashFuncs,
         self.nTweak,
         self.nFlags) = self.__struct.unpack(f.read(self.__struct.size))

    def serialize(self):
        r = serialize.ser_string(self.vData)
        return r + self.__struct.pack(self.nHashFuncs, self.nTweak,
                                      self.nFlags)
